...
def body(sgindex: int):
    ###############################################################################
    logger.info("This code produces fraction scan plots for multiple taggers and datasets")
    ...
    sigeffindex=sgindex
    def calc_rejs(fc_value: float,tag_prob: float, findex: int, tagindex: int,sig_eff: float,sigindex: int):
        ...
        ujets_rej = calc_rej(disc[is_b[findex]], disc[is_l[findex]], sig_eff[sigindex])
        cjets_rej = calc_rej(disc[is_b[findex]], disc[is_c[findex]], sig_eff[sigindex])
    ...
    for i in range(0,len(file_list)):
        ...
        for tagger in range(0,len(tagger_list)):
            sub.append(
                np.array([calc_rejs(fcval,tag_prob,i,tagger,sig_eff,sigeffindex) for fcval in fc_values])
            )
        rej_results.append(sub)
    ...
    for i in range(0,len(file_list)):
        ...
        for tagger in range(0,len(tagger_list)):
            sub1.append(
                np.array(rej_results[i][index][:, 2])
            )
            sub2.append(
                np.array(rej_results[i][index][:, 1])
                )
    ...
    for index,tagger in enumerate(tagger_list):
        ...
        for i in range(0,len(file_list)):
            ...
            """
        # Adding labels
        frac_plot.ylabel = "Light-flavour jets rej"
        frac_plot.xlabel = "$c$-jets rej"

        # Draw and save the plot
        frac_plot.draw()
        frac_plot.savefig("FractionScanPlot_"+str(tagger)+str("_WP"+str(int(100*sig_eff[sigeffindex])))+"_rej_ttb.png")
        del frac_plot
sigeff = [0,1,2,3]
for stuff in sigeff:
    body(stuff)

...
for wpindex in range(0,len(wp)):
    for boolindex in range(0,len(fixedornot)):
        for tagger in range(0, len(tagger_list)):
            ...
            st = f"Internal\nZ'ext; WP{int(100*wp[wpindex])} {fnamesuffix[boolindex]}; $f_{{c}}={fcval}$\nPFlow jets;$\sqrt{{s}}$=13TeV\n$p_T \epsilon [250GeV,6TeV]$"
            logger.info(s1)
            logger.info("Initializing plots as a function of pt.")
            plots_l = []
            plots_c = []
            plot_labels = [" 800030 (R21 Calibration)"," 800030 (R22  Calibration)"]
######################################################################
            for i in range(len(file_list)):
                plots_l.append(
                    VarVsEff(
                        ...
                        bins=[
                            250,500,750,1000,1250,1500,2000,2500,3000,3500,4000,4500,5000,5500,6000,
                        ],
...
######################################################################
            ...
            plot_bkg_rej_l.draw()
            plot_bkg_rej_l.savefig(f"{name1}_pt_vs_l_rej_WP{int(100*wp[wpindex])}_{fnamesuffix[boolindex]}_zp.png")
...
######################################################################
            ...
            plot_bkg_rej_c.draw()
            plot_bkg_rej_c.savefig(f"{name1}_pt_vs_c_rej_WP{int(100*wp[wpindex])}_{fnamesuffix[boolindex]}_zp.png")
######################################################################
            ...
            plot_sig_eff.savefig(f"{name1}_pt_vs_b_eff_WP{int(100*wp[wpindex])}_{fnamesuffix[boolindex]}_zp.png")
